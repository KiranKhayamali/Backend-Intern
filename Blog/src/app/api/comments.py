...
from ..schemas.comment import CommentRead, CommentCreate, CommentCreateInternal, CommentUpdate, CommentUpdateInternal
...
async def write_comment(post_id:int, comment:CommentCreate, current_user:Annotated[SessionDep, Depends(get_current_active_user)], db:SessionDep) -> dict[str, Any]:
    ...
    created_comment = await crud_comments.create(db=db, object=comment_internal, schema_to_select=CommentRead)
...
async def read_comments_from_all_posts(db:SessionDep, page: int = 1, items_per_page: int = 10) -> dict[str, Any]:
    ...
    response: dict[str, Any] = paginated_response(crud_data=comments_data, page=page, items_per_page=items_per_page) 
    return response


@router.put("/comments/{comment_id}", response_model=CommentRead, status_code=201)
async def update_comment(comment_id:int, comment:CommentUpdate, current_user:Annotated[SessionDep, Depends(get_current_active_user)], db:SessionDep) -> dict[str, Any]:
    db_user = await crud_users.get(db=db, username=current_user["username"], is_deleted=False, schema_to_select=UserRead)
    if not db_user:
        raise NotFoundException("User Not Found!!!")
    ...
    db_comment = await crud_comments.get(db=db, id=comment_id, is_deleted=False, schema_to_select=CommentRead)
    ...
    updated_comment = await crud_comments.update(db=db, id=comment_id, object=comment_internal, schema_to_select=CommentRead, return_as_model=True)
...
async def delete_comment(comment_id:int, current_user:Annotated[SessionDep, Depends(get_current_active_user)], db:SessionDep) -> dict[str, str]:
    db_user = await crud_users.get(db=db, username=current_user["username"], is_deleted=False, schema_to_select=UserRead)
    if not db_user:
        raise NotFoundException("User Not Found!!!")
