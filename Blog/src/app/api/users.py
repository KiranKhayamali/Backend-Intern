...
async def read_current_user(
    current_user: Annotated[UserReadWithPost, Depends(get_current_active_user)],
    db: SessionDep,
) -> dict[str, Any]:
...
async def read_user(username_or_email: str, db: SessionDep) -> dict[str, Any]:
    if "@" in username_or_email:
        db_user = await crud_users.get(
            ...
            is_deleted=False,
            schema_to_select=UserReadWithPost,
        )
    else:
        db_user = await crud_users.get(
            db=db,
...
async def create_user(user: UserCreate, db: SessionDep) -> dict[str, Any]:
...
async def update_user(
    ...
    if "@" in username_or_email:
        db_user = await crud_users.get(
            db=db, email=username_or_email, is_deleted=False, schema_to_select=UserRead
        )
    else:
        db_user = await crud_users.get(
            db=db,
    ...
    if "username" in user_update_dict and user_update_dict["username"] != old_username:
        username_row = await crud_users.exists(
            db=db, username=user_update_dict["username"]
        )
        if username_row:
            raise DuplicateValueException("Username not available!!!")

    if "password" in user_update_dict:
        user_update_dict["hashed_password"] = get_password_hash(
            password=user_update_dict["password"]
        )
        del user_update_dict["password"]

    user_update_dict["updated_at"] = datetime.now(UTC)
    user_update_internal_dict = UserUpdateInternal(**user_update_dict).model_dump(
        exclude_unset=True
    )
